<template>
  <main>
    <div>
      <h1>Операторская. Обработка обьяслений</h1>
      <br>
      <button @click="setClosePosition()">попытка сохранить</button>

      <div v-if="isNeedAvito">


      </div>
      <div v-else>
        <div style="float: right" v-if="operatorName">Оператор: {{ operatorName }}</div>
        <div v-else><br>Чтобы работать по сети взаимодействуя с остальными операторами, необходимо открыть сайт <a
            href="https://dev.autonet.pro/">dev.autonet.pro</a> и зарегистрироваться
        </div>
      </div>
    ...
    <!--    <button @click="askData()" v-else>Получить со страницы данные</button>-->
    <!--    <button @click="askAccount()">настраивание аккаунта</button>-->
    <small v-if="isTruePage">
      Нужно перейти на вкладку со списком автомобилей на <a href="https://www.avito.ru/kazan/avtomobili?radius=100">Avito</a>
    </small>
  </main>
</template>
...
chrome.runtime && chrome.runtime.onMessage.addListener(function (message) {
  switch (message.action) {
    case "pass":
      ...
      console.log('avitoData.value', avitoData.value)
      console.log('operatorName.value', operatorName.value)
      break
    default:
      console.log('DEFAULT')

      console.log('__________________options____ message - - -', message)
  }
})
...
function keepInBD(id) {
  sendQustionToContent({}, '3. ' + id)
}
...
function toScrollIntoView(id) {
  sendQustionToContent({currentWindow: true, active: true}, id)
}

// function askData() {
//   console.log('askData')
//   isTruePage.value = true
//   sendQustionToContent({currentWindow: true, active: true}, '1. Parsing site', answerAutoList)
// }

// console.log('--chrome.runtime', chrome.runtime)

// chrome.runtime.sendMessage({site:'От опций',time:'222222'}); // отправка сообщения сервисворкеру
...
function sendQustionToContent(params, question, callback) {
  ...
  console.log('chrome.tabs.query', chrome.tabs.query)
  // нужно переделеать
}
...
async function eee2() {
  const response = await fetch('https://dev.autonet.pro/api/comment//', {
    ...
    method: 'POST',
    body: '{"text":777,"EntityId":391870,"entityType":20}',
  })
